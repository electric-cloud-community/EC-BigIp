# Version: Mon Oct  7 18:12:05 2019
#
...
sub set_output_parameter {
    ...
    eval {
        ...
        if ($is_set && !$is_set eq '0E0') {
            $self->logger->info(
                "Output parameter '$name' has been set to '$value'" . (defined $attach_params ? "and attached to " . Dumper($attach_params) : ''));
        }
    };
    if ($@) {
...
sub run_step {
    ...
    $self->debug_level($dl_saved);

    if (!$ok || $eval_error) {
        $self->bail_out($eval_error || 'Unknown error');
    }
...
sub run_command_with_timeout {
    ...
    return $res;
} ## end sub run_command_with_timeout
...
sub get_config_values {
    ...
    if (!$self->{_config} || $renew) {
        ...
        } ## end for my $node ($properties...)

        $self->{_config} = $retval;
    } ## end if (!$self->{_config} ...)

    return $self->{_config};
} ## end sub get_config_values
...
sub build_metadata_location {
    ...
    return $location;
} ## end sub build_metadata_location
